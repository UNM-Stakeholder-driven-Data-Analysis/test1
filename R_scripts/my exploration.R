...
# format date/time
dat$Datecombined = as.POSIXct(dat$Datecombined, format="%m/%d/%y", tz="MST")
...
dat_lts_phos = 
  ...
  arrange(Datecombined)
# add year and day of year for plotting
dat_lts_phos$year = lubridate::year(dat_lts_phos$Datecombined)
dat_lts_phos$day = lubridate::day(dat_lts_phos$Datecombined)
# plot
ggplot(data=dat_lts_phos, aes(x=Day, y=Phosphate.P..mg.L., color=Site.Name))+
  geom_point() + geom_path()+
  facet_wrap(~year, scales="free_y")+
  theme(legend.title = element_blank()) +
  theme_bw()
# can also look at single years in detail - Not sure where am messing
ggplot(data=dat_lts_phos, aes(x=Year, y=Phosphate.P..mg.L., color=Site.Name))+
  geom_point() + geom_path()+
  xlim(c(as.POSIXct("2005"), as.POSIXct("2016")))+
  theme(legend.title = element_blank()) +
  theme_bw()
...
temp = dat_r[dat_r$Sampling.type == "Ammonia",]
...
temp = dat_r[dat_r$Sampling.type == "Bicarbonate",]
qqPlot(temp$Value); shapiro.test(temp$Value) # normal
qqPlot(temp$Value[temp$Site_Name=='VR-1']); shapiro.test(temp$Value[temp$Site_Name=='VR-1']) # no data
qqPlot(temp$Value[temp$Site_Name=='VR-2']); shapiro.test(temp$Value[temp$Site_Name=='VR-2']) # normal
